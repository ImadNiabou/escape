...
export const SearchBar = ({ placeholder }: { placeholder?: string }) => {
  ...
  const [numOfGuests, setNumOfGests] = useState(1);
  ...
  return (
    <>
      <div className="flex items-center md:border-2 shadow-sm rounded-2xl outline-[#4F719E] p-1 ">
        <input
          type="text"
        ...
        <div>
          <Link
            ...
            onClick={() => setInput("")}
            className="flex-grow text-[#4F719E]"
          >
            <SearchIcon className="w-[35px] text-white bg-[#4F719E] cursor-pointer rounded-xl p-2" />
          </Link>
        </div>
      </div>
      {input && (
